...
describe ParkingLot do
    describe 'initialize' do
        ...
        context 'given input paramaters have partial or no parameters' do
          it 'throws exception due to missing argument errors' do
            expect { ParkingLot.new('')}.to output("Invalid inputs to create slots\n").to_stdout
          end
        end

        context 'given paramaters have invalid inputs' do
          it 'gives error message invalid inputs' do
            expect {ParkingLot.new('sonali')}.to output("Invalid inputs to create slots\n").to_stdout
          end
        end
    end
    ...
    describe 'park' do
        ...
        context 'given input is correct but duplicate vehicle number' do
          it 'logs errors' do
            parking_lot = ParkingLot.new(2)
            parking_lot.park('car_no', 'car_color')
            expect {parking_lot.park('car_no', 'car_color')}.to output("This vehicle is already parked\n").to_stdout
          end
        end
